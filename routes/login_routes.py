...
import sqlite3
from database_layer import DB_PATH
...
@login_routes.route('/delete_all_data')
def delete_all_data():
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute("DELETE FROM users")
    cursor.execute("DELETE FROM purchases")
    cursor.execute("DELETE FROM flavor_count")
    cursor.execute("DELETE FROM simulation_type")
    cursor.execute("DELETE FROM truck_report")
    cursor.execute("DELETE FROM total_purchases")
    cursor.execute("DELETE FROM simulation_type_ref")
