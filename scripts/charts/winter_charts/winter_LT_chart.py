...
import matplotlib
matplotlib.use('Agg')
from scripts.DB_weather_type_query import winter_LT_query

def winter_LT_chart():
    data = winter_LT_query() 
    if not data:
        data = [0, 0, 0, 0, 0]
    ...
    plt.title('Winter Low Time Chart')
    plt.grid()
    plt.savefig('static/charts/Winter_LT_chart.png')
    plt.close()