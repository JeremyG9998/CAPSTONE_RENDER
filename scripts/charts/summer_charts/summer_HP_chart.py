...
import matplotlib
matplotlib.use('Agg')
from scripts.DB_weather_type_query import summer_HP_query

def summer_HP_chart():
    data = summer_HP_query() 
    if not data:
        data = [0, 0, 0, 0, 0]
    ...
    plt.title('Summer High Probability Chart')
    plt.grid()
    plt.savefig('static/charts/Summer_HP_chart.png')
    plt.close()