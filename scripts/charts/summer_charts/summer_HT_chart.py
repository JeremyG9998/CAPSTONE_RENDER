...
import matplotlib
matplotlib.use('Agg')
from scripts.DB_weather_type_query import summer_HT_query

def summer_HT_chart():
    data = summer_HT_query() 
    if not data:
        data = [0, 0, 0, 0, 0]
    ...
    plt.title('Summer High TIme Chart')
    plt.grid()
    plt.savefig('static/charts/Summer_HT_chart.png')
    plt.close()